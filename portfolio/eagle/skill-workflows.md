...
每條流程用到哪些 skill、為什麼這樣設計，下面一條一條說（需要時用 `/showFlow` 隨時印出流程圖）。另外還有一層**貫穿四條流程的支撐層**，讓它們「不靠人記、可交接」，最後再談。
...
- `/pull-frontend`：拉兩個前端專案（官網、後台）最新進度
- `/build-ui-index`：掃前端程式碼，建立「UI 元件 ↔ 後端 API」索引
- `/api-flow-architecture`：建立後端 API 結構文件（Entity / DTO / Service / Controller）
- `/review-api-flow`：前後端對齊檢查，產出問題清單（❌/⚠️），當作 A-2 的彈藥庫
- `/dpf`：開票前主動修的進入點——吃 `/review-api-flow` 的問題清單，直接生 proposal（不需工單規格）
- `/exportN`：把工單匯出成本地 bug 規格
- `/debugP`：連上資料庫分析 bug，產出 proposal
- `/add-pi`：把該模組已知的潛在問題一併納入 proposal
- `/reviewDoc`：檢核提案品質（含程式碼品質、資料流交叉檢核），報告後停下等確認
- `/rrDoc`：依檢核建議修正提案後再檢核一次
- `/implement`：依 proposal 實作，自動檢核 + build 驗證
- `/check-result`：切到 dev/staging DB，用 local API 驗證修正結果
- `/gcommit-push`：更新 proposal + commit + 推送
- `/fxxxf2e`：從 proposal 抽出前端要怎麼改，產生給前端的罐頭留言
...
- `/verifyOWS`：v2 實作後，把 v1 API 與 v2 API 逐欄位比對，確認輸出一致
- `/verify-QA`：欄位映射修好後，三方比對「v2 API ↔ 大後台 API ↔ DB 原始值」，以官網實際顯示的欄位為基準，逐欄位標結果並寫成驗證文件
...
- **UI-API 索引當橋樑**：`/build-ui-index` 掃前端 Vue 程式碼建出的索引，讓我能從「畫面上哪個欄位怪」一路反查到「後端哪支 API、哪張 DB 表」。
- **用後端的能力幫前端 debug**：`/verifyUI` 直接讀前端的綁定邏輯（`{{ data.xxx }}`），串起「DB 原始值 → API 回傳值 → 前端該顯示什麼」做三方比對。即使不是前端專家，也能精準指出是「後端 API 給錯」還是「前端綁錯欄位」，甚至能抓出「API 正確回傳、但前端根本沒用這個欄位」的隱性問題。
- **把跨團隊溝通也自動化**：修完之後 `/fxxxf2e` 把「前端該配合改什麼」整理成可直接貼給前端工程師的留言。

### 單行文字流程圖

```
/pull-frontend → /build-ui-index → /run-frontend → /verifyUI {module} → /fxxxf2e
```

### 各 skill 用途

- `/pull-frontend`：拉兩個前端專案最新進度
- `/build-ui-index`：掃前端，建立「UI 元件 ↔ 後端 API」索引（那座橋）
- `/run-frontend`：本地啟／停前端 dev server，重現問題
- `/verifyUI`：讀前端綁定邏輯，端到端比對「DB → API → 前端顯示」，逐欄位標 ✅/❌
- `/fxxxf2e`：抽出前端修改建議，產生給前端的罐頭留言
...
- `/fix-id-string-number`：掃 DTO 的 id 欄位是否缺型別轉換，直接自動修正 + build 驗證（低複雜度）
- `/fixPermissionError`：掃 Service + Controller 的權限錯誤處理，產出修復 proposal，再走核心段人工把關（高複雜度）
...
- **`/showFlow` + proposal 置頂進度表**：流程隨時可視化；進度表由各 skill 自動更新 ✅/👉，接手的人一眼看到走到哪。
- **設計稿維護（`/updateDesign`、`/syncCompact`）**：每個 skill 都有一份設計稿 + 一張流程圖，整套配置版本控制、改動自動同步備份——確保這套系統「不是只有我會用」。
- **知識按需載入（`/guideArchitecture`、`/findDoc`）**：開發前才載入對應領域的架構知識（權限、檔案上傳、異動紀錄等），用完即丟，也讓同一套知識能跨專案共用。
