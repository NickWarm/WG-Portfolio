...
與其去防，不如像教 [Word, PPT 的台科大林孟彥教授](https://www.youtube.com/watch?v=1n2wrfktKAk)一開始就把投影片無償準備給學生一樣

大師平台可以請軟體工程師打通這些環節，抽出音訊並上傳到 NotebookLM，讓購買同個課程的學生，都可以問同一個 NotebookLM 問題。

學生先跟 NotebookLM 問過自己好奇的問題後，讓學生心中帶著問題來看老師在大師平台的課程影片，這樣完課與學習效率會提高。
...
要開始自動化，最快的方式就是研究已經有什麼 open source 工具可以用。

就像創業從 0 到 1 最理想的方式「是用抄的」，從 1 到 100 才是差異化。

從 0 到 1 的時候，完全可以用 Gihub cli 讓 Claude Code 去研究類似的情境，有沒有人解過。

就算人家只解了目標流程的一小塊，但是從 open source 已有的資源開始研究，也比自己從 0 開始想來得快

這也是為何我在履歷最前面，要先說，我要讓 Claude Code 使用 Github CLI (gh)，這樣就能讓 Claude Code 快速研究然後找出可能的方向與策略
...
每間公司，需要自動化的痛點又都不同，做自動化這份工作的軟體工程師，應該要很有興趣公司裡「所有職位」的工作項目。

只有實際去投入業務、行銷、客服...，各種跟傳統軟體工程師不同的工作內容，去反思並且與同事討論哪邊可以自動化，然後再用駭客的思考方式，去找出程式與 AI 可以自動化的環節，這樣才會落地。

而且好的自動化，應該是 20 分時就開始給同事使用，然後再透過同事的反饋打磨，從 20 分到 60 分的過程，工程師也要分享「如何思考自動化」與「如何對 AI 提問」，給自動化使用的目標同事(業務、行銷、客服...)

同事在「使用AI」這件事，受到工程師的示範與潛移默化，就會知道如何與 AI 討論和改進，再加上自身經驗，就能打造出更好的 AI 自動化流程。

如果所有自動化環節，都要 AI 工程師親自介入，那麼在單位時間腦力與注意力這種稀缺資源有限的情況下，是不可能兼顧到所有能自動化的環節。

但假如打磨出 60 分的自動化流程的過程中，有越來越多不同崗位，有經驗的同事也學會如何與 AI 協作和思考，就有機會把可自動化的環節，在有限時間之內，打磨到 80 分、90 分，這才是合理的策略。
